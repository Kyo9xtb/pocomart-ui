...
import { CartEmty, CartFull } from '~/components/Cart';
...
function Header() {
    ...
    const { Carts, Favourites, Compares } = state;
        ...
        <div className={cx('header')}>
            <div className={cx('container')}>
                <div className={cx('row')}>
                    ...
                    {/*Mobile-btn*/}
                    <div className={cx('d-lg-none d-block col-2 order-3', 'btn-menu-mobile')}>
                    ...
                    {/*Search-box */}
                    <div className={cx('col-lg-8 col-xl-7 col-md-5 col-10 order-3 order-md-2', 'col-search')}>
                        <div className={cx('theme-search-box')}>
                    ...
                    {/*Account */}
                    <div className={cx('col-lg-2 col-md-3 col-6 order-2 order-md-3', 'col-account')}>
                        <ul className={cx('group-account')}>
                            ...
                            <li>
                                <div className={cx('icon')}>
                                    <Link to="/yeu-thich">
                                        <svg
                                            style={{ enableBackground: 'new 0 0 412.735 412.735' }}
                                            version="1.1"
                                            viewBox="0 0 412.74 412.74"
                                            xmlSpace="preserve"
                                            xmlns="http://www.w3.org/2000/svg"
                                        >
                                            {' '}
                                        ...
                                        <span>
                                            {Favourites.length !== 0
                                                ? Favourites.reduce((total, product) => total + 1, 0)
                                                : 0}
                                        </span>
                                    </Link>
                                </div>
                            ...
                            <li className={cx('cart-drop')}>
                                <div className={cx('icon')}>
                                    <Link to="/cart">
                                        ...
                                        <span>
                                            {Carts.length !== 0
                                                ? Carts.reduce((total, product) => total + product.quantily, 0)
                                                : 0}
                                        </span>
                                    </Link>
                                </div>
                                <div className={cx('top-cart-content')}>
                                    <div className={cx('CartHeaderContainer')}>
                                        {Carts.length !== 0 ? <CartFull data={Carts} /> : <CartEmty />}
                                    </div>
                                </div>
                            </li>
                            {/*Shuffle */}
                            <li>
                                <div className={cx('icon')}>
                                    <Link to="/so-sanh" title="So sánh">
                                        ...
                                        <span>
                                            {Compares.length !== 0
                                                ? Compares.reduce((total, product) => total + 1, 0)
                                                : 0}
                                        </span>
                                    </Link>
                                </div>
