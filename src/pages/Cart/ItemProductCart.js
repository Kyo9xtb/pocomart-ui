...
function ItemProductCart({ data }) {
    const { id, image_url, product_name, price, quantily, sale } = data;
    const [amount, setAmount] = useState(quantily);
    const [, dispatch] = useStore();
    const handleIncrease = (e) => {
        e.stopPropagation();
        if (amount < 20) {
            setAmount(amount + 1);
            data.quantily = amount + 1;
    ...
    useEffect(() => {
        ...
        if (getProduct.price * (1 - getProduct.sale / 100) !== price * (1 - sale / 100)) {
            setCheckPrice(true);
        }
    }, []);
    return (
